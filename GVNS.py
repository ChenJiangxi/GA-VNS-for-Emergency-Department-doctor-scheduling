...
#                 start_last = gen_solution.find_day_ind(day, doctor.start, 0)
#                 end_last =  gen_solution.find_day_ind(day, doctor.end, 1)
#                 if end_last + 2 <= (day+1)*24-4 and end_last - start_last < 7:
#                     start =  gen_solution.gen_prob_randint(solution.length, end_last + 2, (day+1) * 24 - 4)
#                 elif start_last - 2 >= day*24 + 7 + 4 and end_last - start_last < 7:
...
#                 discard_list = []
#                 for h in range(6):
#                     discard_list.append(day*24 + 1 + h)
...
#                     end =  gen_solution.get_end(day, start, doctor)
#                     if end:
#                         doctor.start.sort()
#                         doctor.end.sort()
#                         if solution.get_objection() < best_obj -0.1:
...
def reduce_doc(init_solution): #减少人数
    solution = copy.deepcopy(init_solution)
    print("red_doc_before:", solution.schedule)
    while True:
        ...
        if 3 in solution.doc_list[i].dayornight:
            ...
            print("red_doc_after:", solution.schedule)
            return solution
...
def VNS(initial_solution, l_max = 4, k_max = 4, iteration = 10):
    ...
    # plot_obj(objection_list)
    while(count < iteration or not  best_solution.len_flag):
        k = 0
        while(k < k_max):
            if k == 0:
                x1 = reduce_doc(best_solution)
            else:
                x1 = rearrange(best_solution, k)
            # objection_list.append(x1.obj)
            # plot_obj(objection_list)
            x2 = VND(x1, l_max)
            for doc in x2.doc_list:
                doc.check()
            ...
            if best_solution.len_flag == False or x2.len_flag == True:
                ...
                while True:
                    ...
                    if x2.get_objection() == obj:
                        ...
                        obj = x2.get_objection()
                # objection_list.append(x2.obj)
                # plot_obj(objection_list)

            for doc in x2.doc_list: #check
                doc.check()

            decay_factor = gen_solution.exponential_decay(count, init = 1, m = 10, finish = 1.01)
            if (x2.get_objection() < best_solution.get_objection() * decay_factor):
                best_solution = copy.deepcopy(x2)
                gen_solution.store(best_solution, best_solution.obj, 'temp store week2/')
                print("temp best shcedule =", best_solution.schedule)
                k = 0
            else:
                k += 1
        plot_len_contrast(initial_solution.length, best_solution.length)
        print("Iteration = ", count,  "Objection =", best_solution.obj, "max_len =", max(best_solution.length), "doctor_num =", best_solution.__len__(),"len_flag", best_solution.len_flag)
        count += 1
        initial_solution.iter = count
...
if __name__ == '__main__':
    with open('temp store week1/solution579.pk', 'rb') as f:
        initial_solution = pickle.load(f)
    objection_list = []
    objection_list.append(initial_solution.get_objection())
